...
function CommentOverlay({ open, onClose, track }) {
  ...
  const fetchComments = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.get(`${API_URL}/api/comments/track/${track._id}/comments-hierarchical`);
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/api/comments/track/${track._id}/add`, {
  ...
  const handleAddReply = async (parentId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${API_URL}/api/comments/comment/${parentId}/reply`, {
        trackId: track._id,
        text: replyText,
      }, { headers: { Authorization: `Bearer ${token}` } });
